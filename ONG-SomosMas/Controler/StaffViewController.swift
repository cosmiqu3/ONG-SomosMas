...
class StaffViewController: BaseViewController, UICollectionViewDelegate, UICollectionViewDataSource,
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = stafCollection.dequeueReusableCell(withReuseIdentifier: "celdaStaff", for: indexPath) as! CeldaCollectionViewCell
        
        if let imagePhoto = UIImage(named: staffList[indexPath.row].image) {
            
            //cell.imagen.image = imagePhoto
            let pointSize = CGPoint(x: 2, y: staffList[indexPath.row].letterPosition)
            let textoImagen: String = staffList[indexPath.row].name
            cell.imagen.image = textToImage(drawText: textoImagen as NSString, inImage: imagePhoto, atPoint: pointSize, letra: staffList[indexPath.row].letterSize)
        }
